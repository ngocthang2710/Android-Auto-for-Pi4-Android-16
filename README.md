...
scoped to wired-USB development only**; it now carries the wireless
code/docs as a merged, working snapshot, but further wireless work
continues on its own branch rather than here.
...
- Touch input on the head-unit screen, reliably registered on the phone
  (see [Fixed](#fixed) below).
- Recovers on its own from a transport that's died silently (repeated
  SSL errors, no ping response, no USB detach broadcast) instead of
  freezing on the last frame until manually replugged -- a native
  watchdog in `AndroidAutoEntity` flags the session fatally dead after
  ~8s of no ping response, polled from `AaSdkUsbService` and torn down
  the same way a real accessory detach is.
...
**Fixed:**
- **Touch input previously produced no reaction on the phone.** Root cause
  turned out to be a coordinate-space mismatch, not a protocol bug: GearHead
  (the phone-side AA app)'s own window manager locks its touch window to
  `video_configs[0]`, which was still advertising an old 800x480 tier while
  `VideoService`/`AaSdkScreenActivity` operated in 1920x1080 -- every touch
...
- `AndroidVideoOutput` -- NDK `AMediaCodec` decoding into a permanent
  `SurfaceTexture` (via `VideoBlitter.kt`, not a raw `ANativeWindow`
  anymore), which is then GL-blitted onto whatever on-screen `Surface`
  `AaSdkScreenActivity` currently provides. The decoder's own output
  target never changes across a screen re-entry or session reconnect,
  which is what makes those safe -- see `VideoBlitter`'s kdoc.
