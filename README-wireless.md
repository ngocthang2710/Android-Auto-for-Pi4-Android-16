...
way to a rendered Android Auto screen that stays up under video + audio +
...
The follow-up patch series that took this from "renders once, then dies" to
a **stable 5GHz session** (regulatory/HAL/channel fixes for the 5GHz AP, an
...
- **`app/src/main/java/com/android/car/aasdk/AaSdkSoftApHotspot.kt`** -- hosts
  ...
  IP for the interface -- see [Debugging log](#debugging-log-what-broke-and-why)).
...
entirely on the separate TCP socket; a connection whose handshake actually
completed owns tearing the AP back down when its BT link ends -- see
[docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md#5-reconnect-flow-fixes-storms-wedges-livelocks)
for why that ownership had to become conditional.

### `AaSdkSoftApHotspot.kt` -- the self-hosted WiFi AP

Hosts a fixed-configuration AP (SSID `AndroidAuto-HU`, WPA2-PSK passphrase
`aasdkwireless`, MAC randomization disabled so the BSSID handed to the phone
over BT stays valid) via `TetheringManager.startTethering()`. 5GHz (channel
36/5180MHz, pinned) is the primary band, with a 2.4GHz fallback if it fails
-- see [docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md#1-the-5ghz-chain-three-stacked-blockers)
for why 5GHz needed three separate fixes (regulatory domain, vendor WiFi HAL,
channel selection) to actually come up, and why it matters (phones abort
projection on a 2.4GHz-only link).

Two non-obvious things this class has to get right, each one a bug found by
...
(start when a phone BT-connects, stop when its *completed* BT handshake's
link ends), not from `AaSdkUsbService.onCreate()`/`onDestroy()`. This matters
because the Pi4's WiFi radio is single-radio: switching it into AP mode
unconditionally on service startup would mean a plain wired-USB-only session
(which never touches WiFi at all) would still silently commandeer the WiFi
radio away from station mode (e.g. dropping `adb`-over-WiFi) every time a
phone is plugged in. Gating AP startup on an actual incoming BT wireless
request keeps the two paths independent.

`start()` is idempotent and self-healing (guarded by an `isActive` flag with
a wedge-detection timeout) so a phone's BT retry storm racing an AP that's
already up, still coming up, or stuck from a failed prior attempt doesn't
leave wireless AA dead until a service restart.
...
- **`MessageInStream`** reassembles frames **per channel** rather than in one
  shared buffer -- wireless TCP legally interleaves other channels' frames
  (e.g. audio) between a split video message's FIRST/LAST frames, which the
  upstream single-buffer implementation rejected outright and which used to
  freeze every wireless session ~20s in. See
  [docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md#2-the-freeze-aasdk-cannot-parse-interleaved-channel-frames-the-deepest-bug).
...
real hardware against a real phone (this covers getting a session to render
at all; the later work that took a working session to a *stable* one --
5GHz, the interleaved-frames freeze, reconnect loops -- is its own document,
[docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md)):
...
2. **AP always `WIFI_AP_STATE_FAILED` on 5GHz**: country-code/regulatory
   domain issue. Initially worked around by switching to `BAND_2GHZ`; the
   real fix (regulatory domain + vendor HAL + channel selection) came later,
   see [docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md#1-the-5ghz-chain-three-stacked-blockers).
...
The stabilization series (5GHz WiFi HAL fix, etc.) adds two more --
see [docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md#complete-file-inventory)
for the complete file-by-file inventory of everything this project touches.
...
See [docs/systemui-bluetooth-crash-fix.md](docs/systemui-bluetooth-crash-fix.md)
for why, and [docs/wireless-stabilization-fixes.md](docs/wireless-stabilization-fixes.md#build--flash)
for the additional `platform-patches/wifi_hal.cpp.patch` the stabilization
series needs on top.
